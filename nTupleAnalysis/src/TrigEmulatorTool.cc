...
TrigEmulatorTool::TrigEmulatorTool(std::string name, int mode, int nToys, bool debug) :  m_name(name),  m_nToys(nToys), m_mode(mode), m_debug(debug)
{
  ...
  for(unsigned int i = 0; i < m_PFJet_trigNames.size(); ++i){
    ...
    m_PFJet.insert(std::make_pair(m_PFJet_trigNames.at(i),new HLTJetEmulator("PFJ"+m_PFJet_trigNames.at(i),m_PFJet_fileName,m_PFJet_histNames.at(i),debug)));
  }
...
void TrigEmulatorTool::SetDecisions(std::vector<float> offline_jet_pts, std::vector<float> offline_btagged_jet_pts, float ht){
...
void TrigEmulatorTool::SetWeights(std::vector<float> offline_jet_pts, std::vector<float> offline_btagged_jet_pts, float ht){
  for (auto trigIt=m_emulatedTrigMenu.begin(); trigIt!=m_emulatedTrigMenu.end(); ++trigIt){
...
void TrigEmulatorTool::Fill(std::vector<float> offline_jet_pts, std::vector<float> offline_btagged_jet_pts, float ht){
  for (auto trigIt=m_emulatedTrigMenu.begin(); trigIt!=m_emulatedTrigMenu.end(); ++trigIt)
